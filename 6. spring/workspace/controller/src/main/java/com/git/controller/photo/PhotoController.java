...
import com.git.controller.lib.TesxProcesser;
...
public class PhotoController {
	...
	public Photo addPhoto(@RequestBody Photo photo, HttpServletResponse res) throws InterruptedException {
		...
		// 타이틀 빈값
		if (TesxProcesser.isEmptyText(photo.getTitle())) {
			res.setStatus(HttpServletResponse.SC_BAD_REQUEST);
			return null;
		}

		// 파일 url이 빈값
		if (TesxProcesser.isEmptyText(photo.getPhotoUrl())) {
			res.setStatus(HttpServletResponse.SC_BAD_REQUEST);
			return null;
		}
		...
		Photo photoItem = Photo.builder().id(currentId).title(photo.getTitle())
				.description(TesxProcesser.getPlainText(photo.getDescription())).photoUrl(photo.getPhotoUrl())
	...
	public Photo modifyPhotos(@PathVariable long id, @RequestBody Photo photo, HttpServletResponse res) {
		...
		photoItem.setDescription(TesxProcesser.getPlainText(photo.getDescription()));
