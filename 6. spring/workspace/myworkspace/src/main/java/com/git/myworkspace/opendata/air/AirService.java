...
//	@Scheduled(cron = "0 0 */2 * * *")	
	...
	public void requestAirSiGunGuHour(String sido) throws IOException {
		...
		// 3.Http 접속 생성 URL객체가 상위타입 이며 (Http 만 있는게 아니라 FTP 같은 프로토콜도 있어서) HTTP로 접속 하려면
		// url을 HTTP접속용 객체로 바꿔야 함
		HttpURLConnection con = (HttpURLConnection) url.openConnection();

		// 4. byte[]배열로 데이터를 읽어옴 ( 데이터를 읽는 가장 기초적인 방식 byte 단위면 1byte씩 읽는다. )
